...
namespace
{
  ...
  // draw the recursion-demo block with a total and a prefix-match count
  void renderRecursionPanel(GuiApplicationState &applicationState)
  {
    ImGui::SeparatorText("Recursion Demo");

    const int totalRecursiveCount = countContactsRecursive(applicationState.contactList);
    ImGui::Text("Total contacts (recursive count): %d", totalRecursiveCount);

    ImGui::InputTextWithHint("Phone prefix", "e.g. +359",
                             applicationState.phonePrefixBuffer,
                             GuiApplicationState::PHONE_PREFIX_BUFFER_SIZE);

    const std::string phonePrefixString = applicationState.phonePrefixBuffer;
    const int matchingPrefixCount = countContactsWithPhonePrefixRecursive(
        applicationState.contactList, phonePrefixString);
